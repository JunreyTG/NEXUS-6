...
type StoredRow = { record_id: string; record_data: string };
...
export class SqlServerAdapter extends UnconfiguredDatabaseAdapter {
  ...
  constructor(configuration = getDatasetDatabaseConfig().SQLSERVER) {
    super();
  ...
  override createStorage(request: StorageRequest): Promise<StorageDescriptor> {
    return this.write(async () => {
      ...
      await pool.request().query(`
        IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = '${validateIdentifier(request.storageIdentifier)}')
        BEGIN
          CREATE TABLE ${table} (record_id NVARCHAR(191) NOT NULL PRIMARY KEY, record_data NVARCHAR(MAX) NOT NULL, created_at DATETIME2(3) NOT NULL DEFAULT SYSUTCDATETIME(), updated_at DATETIME2(3) NOT NULL DEFAULT SYSUTCDATETIME())
        END
      `);
  ...
  override getRecord(request: RecordRequest): Promise<DatabaseRecord | null> {
    return this.read(async () => {
      const table = identifier(request.storageIdentifier);
      const pool = await this.getPool();
      const result = await pool.request().input("recordId", sql.NVarChar, request.recordId).query<StoredRow>(`SELECT TOP 1 record_id, record_data FROM ${table} WHERE record_id = @recordId`);
      const row = result.recordset[0];
      return row ? recordWithId(row.record_id, JSON.parse(row.record_data)) : null;
    });
  }
  ...
  override insertRecord(request: InsertRecordRequest): Promise<DatabaseRecord> {
    return this.write(async () => {
      ...
      const table = identifier(request.storageIdentifier);
      const pool = await this.getPool();
      await pool.request().input("recordId", sql.NVarChar, recordId).input("recordData", sql.NVarChar(sql.MAX), JSON.stringify(request.record)).query(`INSERT INTO ${table} (record_id, record_data) VALUES (@recordId, @recordData)`);
  ...
  override updateRecord(request: UpdateRecordRequest): Promise<DatabaseRecord> {
    return this.write(async () => {
      const table = identifier(request.storageIdentifier);
      const pool = await this.getPool();
      await pool.request().input("recordId", sql.NVarChar, request.recordId).input("recordData", sql.NVarChar(sql.MAX), JSON.stringify(request.record)).query(`UPDATE ${table} SET record_data = @recordData, updated_at = SYSUTCDATETIME() WHERE record_id = @recordId`);
  ...
  override deleteRecord(request: RecordRequest): Promise<void> {
    return this.write(async () => {
      const table = identifier(request.storageIdentifier);
      const pool = await this.getPool();
      await pool.request().input("recordId", sql.NVarChar, request.recordId).query(`DELETE FROM ${table} WHERE record_id = @recordId`);
    });
  }
  ...
  override deleteStorage(request: StorageRequest): Promise<void> {
    return this.write(async () => {
      const table = identifier(request.storageIdentifier);
      const pool = await this.getPool();
      await pool.request().query(`IF OBJECT_ID(N'dbo.${validateIdentifier(request.storageIdentifier)}', N'U') IS NOT NULL DROP TABLE ${table}`);
    });
  }

  private async rows(storageIdentifier: string): Promise<DatabaseRecord[]> {
    const table = identifier(storageIdentifier);
    const pool = await this.getPool();
    const result = await pool.request().query<StoredRow>(`SELECT record_id, record_data FROM ${table} ORDER BY created_at ASC, record_id ASC`);
    return result.recordset.map((row) => recordWithId(row.record_id, JSON.parse(row.record_data)));
  }

  private getPool(): Promise<ConnectionPool> {
    if (!this.configuration.configured || !this.configuration.host || !this.configuration.database || !this.configuration.user || !this.configuration.password) return Promise.reject(new DatabaseNotConfiguredError(this.engine));
    ...
    const poolPromise = new sql.ConnectionPool({
      ...
      options: { encrypt: this.configuration.encrypt, trustServerCertificate: this.configuration.trustServerCertificate },
...
function descriptor(storageIdentifier: string, databaseName: string | undefined): StorageDescriptor {
  return { engine: "SQLSERVER", storageType: "TABLE", storageIdentifier, ...(databaseName ? { databaseName } : {}), tableOrCollection: storageIdentifier };
}
